...
class Patcher(dispatch: ActorRef) extends Actor {
  ...
  val logger: Logger = Logger.getLogger("PATCHER")
  ...
  override def receive: Receive = {
    case p: ParcelFile =>
      ...
      board(p.from) ! ShowFile(p.filename)
    case p: ParcelPatch =>
      ...
      val patchList = new util.LinkedList[DiffMatchPatch.Patch](dmp.patchFromText(p.patch))
      val a = dmp.patchApply(patchList, curText)
      val newText : String = a.head match { case x: String => x }
      val diagnostics: List[Boolean] = a.tail.head match { case xs: Array[Boolean] => xs.toList }
